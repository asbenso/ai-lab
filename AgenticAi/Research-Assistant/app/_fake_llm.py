...
def _writer_payload_text(human: str) -> str:
    marker = "Research payload:"
    if marker in human:
        return human.split(marker, 1)[1].strip()
    return human.strip()


def _writer_use_bullets(memories: list[dict]) -> bool:
    for item in memories:
        value = item.get("value") or {}
        if value.get("kind") != "preference":
            continue
        content = (value.get("content") or "").lower()
        if "bullet" in content or "list format" in content:
            return True
    return False
...
def _writer_markdown(human: str) -> str:
    ...
    try:
        data = json.loads(_writer_payload_text(human))
    except Exception:
        return f"# Report\n\n{human[:400]}\n"
    ...
    use_bullets = _writer_use_bullets(memories)
